<template>
	<view class="container">
		<view v-for="(item, index) in cellData" :key="index" style="margin-bottom: 1px;">
			<appointmentCell @click="clickItem" :index="index" :title="item.title" :placeholder="item.placeholder"
				:value="item.value" :showArrow="item.showArrow"></appointmentCell>
		</view>
		...
		<view style="margin: 20px 0 20px; padding: 0 10px;">
			<normalBtn text="提交预约" @click="commit"></normalBtn>
		</view>
		...
		<view v-if="showPopupview" class="popupview">
			...
			<view v-if="showPopupview" class="popupview-container fadelogIn">
				<view
					style="flex-shrink: 0; color: #007AFF; font-size: 17px; text-align: center; height: 50px; line-height: 50px;">
					{{tipsText}}
				</view>

				<!-- 部门 -->
				<view v-if="showDeptView" class="popupview-list">
					<view @click="selectDept(item)" class="popupview-list-cell" v-for="(item,index) in deptList"
						:key="index">
						{{item.SHORTNAME}}
					</view>
				</view>
				...
				<view v-if="showBusinessView" class="popupview-list">
					<view @click="selectBusiness(item)" class="popupview-list-cell" v-for="(item,index) in businessList"
						:key="index">
						{{item.SXZXNAME}}
					</view>
				</view>
				...
				<view v-if="showDateView" class="popupview-list date-list">
					<view @click="selectDate(item)" class="date-list-cell" v-for="(item,index) in dateList"
						:key="index">
						{{item.RESERVEDATE}}
					</view>
				</view>
				...
				<view v-if="showTimeView" class="popupview-list date-list">
					<view @click="selectTime(item)" class="date-list-cell" v-for="(item,index) in timeList"
						:key="index">
