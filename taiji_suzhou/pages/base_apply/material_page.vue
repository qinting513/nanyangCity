...
export default {
	...
	data() {
		return {
			tipString:
				'温馨提示：如果材料是暂未上传的标识需要点击上传附件；如果材料出现上传失败的标识，请点击重新上传，直到出现材料上传成功的绿色标识，既表示该材料已成功电子化上传。',
	...
	computed: mapState(['applyItemInfo', 'userInfo', 'businessModel', 'formsModel', 'materialsModel', 'postModel', 'materialList']),
	created() {},
	mounted() {
	...
	methods: {
		...
		checkMaterial: function(showTip) {
			...
			for (let i in this.materialList) {
				...
				if (
					item.SFBY == '1' &&
					(this.materialsNumber[item.ID] == null || this.materialsNumber[item.ID]['uploaded'] == null || this.materialsNumber[item.ID]['uploaded'] == 0)
				) {
					ret = false;
				}
			}
